...
/**
 * World setting for configuring the linked end world for an overworld base world.
 */
public class WorldEndSetting implements WorldBaseSetting {
    private static final String STORAGE_KEY = "end-world";
    ...
    public void onCommand(@NotNull CommandContext ctx, @NotNull ConfigSection config, @NotNull World world) {
        ...
        if (rawValue.isEmpty()) {
            ctx.returnInfo("WORLD_SETTING_END_STATUS", "world", world.getName(), "value", get(world, config));
        }
        String value = rawValue.toLowerCase(Locale.ROOT);

        if (value.equals("unset")) {
            set(world, config, value);
            ctx.returnSuccess("WORLD_SETTING_END_RESET", "world", world.getName());
        }
        ...
        if (WorldUtil.resolveEnvironment(rawValue) != World.Environment.THE_END) {
            ctx.returnError("WORLD_SETTING_END_INVALID_WORLD", "world", rawValue);
        }

        set(world, config, rawValue);
        ctx.returnSuccess("WORLD_SETTING_END_SET", "world", world.getName(), "value", rawValue);
    }
