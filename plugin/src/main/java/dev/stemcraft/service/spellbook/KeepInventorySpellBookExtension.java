...
/**
 * Keeps the player's inventory when they die while carrying the spell book.
 */
public final class KeepInventorySpellBookExtension implements SpellBookExtension {
    private static final String DEFAULT_SPELL = "die keep inventory";
    ...
    public void register(@NotNull SpellBookExtensionContext context) {
        ...
        api.events().register(PlayerDeathEvent.class, event -> {
            ...
            SpellBookMatch match = spellBooks.findSpell(event.getEntity(), source, spell);
            if (match == null) {
                return;
            }
            ...
            SpellBookNegativeEffect.applyConfigured(event.getEntity(), match, config);
