...
final class PunchTheBatCommand {
    private final STEMCraftAPI api;
    private final PunchTheBatMiniGame owner;

    PunchTheBatCommand(STEMCraftAPI api, PunchTheBatMiniGame owner) {
        this.api = api;
        this.owner = owner;
    }

    void register() {
        String id = PunchTheBatMiniGame.NAMESPACE;
        ...
        for (String action : List.of("list", "leave", "create", "info", "join", "spectate", "start", "stop", "save",
                "validate", "enable", "disable", "delete", "addspawn", "removespawn", "addtarget", "removetarget")) {
            command.tabCompletion(action, "{" + id + "-arenas}");
        }
        for (String key : List.of("arena", "lobby", "spectator", "minplayers", "maxplayers", "roundseconds", "countdown", "targetcount")) {
    ...
    private void execute(CommandContext ctx) {
        ...
        if (!publicAction && !ctx.hasPermission("stemcraft.command." + PunchTheBatMiniGame.NAMESPACE) && !ctx.getSender().isOp()) {
            ctx.returnError("Arena setup requires stemcraft.command." + PunchTheBatMiniGame.NAMESPACE);
            return;
        }
        ...
        switch (action) {
            case "info", "validate" -> {
                ctx.info(a.id() + ": " + a.getStatus() + "; spawns=" + owner.settings(a).spawns.size()
                        + "; targets=" + owner.settings(a).targets.size());
        ...
        PunchTheBatArenaSettings d = owner.settings(a);
        switch (action) {
            case "addspawn" -> d.spawns.add(location(ctx, a));
            case "addtarget" -> d.targets.add(location(ctx, a));
            case "removespawn", "removetarget" -> {
                List<?> entries = action.equals("removespawn") ? d.spawns : d.targets;
                ...
                switch (ctx.getArgLower(2)) {
                    ...
                    default -> {
                        ...
                        switch (ctx.getArgLower(2)) {
                            ...
                            case "targetcount" -> d.targetCount = n;
                            default -> {
