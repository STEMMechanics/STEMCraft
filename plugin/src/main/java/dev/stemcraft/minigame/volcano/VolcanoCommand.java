...
final class VolcanoCommand {
    ...
    VolcanoCommand(STEMCraftAPI api, VolcanoMiniGame owner) {
        this.api = api;
        this.owner = owner;
    }

    void register() {
        String id = VolcanoMiniGame.NAMESPACE;
    ...
    private void execute(CommandContext ctx) {
        ...
        if (!publicAction && !ctx.hasPermission("stemcraft.command." + VolcanoMiniGame.NAMESPACE) && !ctx.getSender().isOp()) {
            ctx.returnError("Arena setup requires stemcraft.command." + VolcanoMiniGame.NAMESPACE);
            return;
        }
        ...
        VolcanoArenaSettings d = owner.settings(a);
        switch (action) {
