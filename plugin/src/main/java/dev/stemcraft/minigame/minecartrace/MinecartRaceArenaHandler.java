...
final class MinecartRaceArenaHandler implements MiniGameArenaHandler {
    ...
    MinecartRaceArenaHandler(STEMCraftAPI api, MinecartRaceMiniGame owner) {
        this.api = api;
        this.owner = owner;
    }

    private MinecartRaceArenaSettings settings(MiniGameArena a) {
        return owner.settings(a);
    }

    private String task(MiniGameArena a) {
        return "minigame:" + MinecartRaceMiniGame.NAMESPACE + ":" + a.id();
    }
    ...
    public void validate(@NotNull MiniGameArena a, ArenaValidationResult result) {
        MinecartRaceArenaSettings d = settings(a);
        ...
        for (Location l : d.spawns) {
            if (l != null && !Tag.RAILS.isTagged(l.getBlock().getType()))
                result.addError("Minecart spawns must be on rails (stand on the rail when adding a spawn).", "spawns");
        }
    }
    ...
    private boolean contained(MiniGameArena a, SCRegion r) {
        ...
        for (var v : r.getRegion()) if (!inside(a, new Location(a.world(), v.x(), v.y(), v.z()))) return false;
        return true;
    }
    ...
    public Location onPlayerJoinArena(MiniGameArena a, Player p) {
        ...
        api.messages().info(p, MinecartRaceMiniGame.INSTRUCTIONS);
    ...
    public void onArenaStatusChanged(MiniGameArena a, MiniGameArena.ArenaStatus oldStatus, MiniGameArena.ArenaStatus status) {
        if (status == STARTING) {
            ...
            rounds.put(a, r);
            a.set("winner", "-");
            r.objective = MinecartRaceMiniGame.INSTRUCTIONS;
            int i = 0;
    ...
    private void cleanup(MiniGameArena a) {
        ...
        if (r == null) return;
        r.entities.forEach(Entity::remove);
        for (BlockState state : r.original.values()) state.update(true, state.getType() == Material.LEVER);
    }
    ...
    private void equip(MiniGameArena a, Player p, Round r) {
        ...
        p.setFireTicks(0);
        spawnMount(a, p, r);
        p.getInventory().setItem(0, boostItem());
    }

    private ItemStack boostItem() {
        ItemStack item = new ItemStack(Material.FEATHER);
        var meta = item.getItemMeta();
        meta.displayName(Component.text("Boost (right-click)"));
    ...
    private void spawnMount(MiniGameArena a, Player p, Round r) {
        ...
        Entity mount;
        Minecart cart = a.world().spawn(l, Minecart.class);
        cart.setMaxSpeed(0.65);
        mount = cart;
    ...
    private void raceTick(MiniGameArena a, Player p, Round r) {
        ...
        if (mount instanceof Minecart cart && cart.getVelocity().lengthSquared() < 0.01) {
            Vector forward = mount.getLocation().getDirection().setY(0);
            if (forward.lengthSquared() > 0) cart.setVelocity(forward.normalize().multiply(0.2));
        }
    ...
    private void interact(PlayerInteractEvent event) {
        ...
        Round activeRound = rounds.get(a);
        if (a.hasPlayer(p) && a.getStatus() == RUNNING
                && event.getClickedBlock() != null && inside(a, event.getClickedBlock().getLocation())
                && event.getClickedBlock().getType() == Material.LEVER && activeRound != null) {
            activeRound.original.putIfAbsent(event.getClickedBlock(), event.getClickedBlock().getState());
            return;
        }
        ...
        if (p.getInventory().getItemInMainHand().getType() == Material.FEATHER) {
            Entity cart = r.mounts.get(p.getUniqueId());
            if (cart == null || p.getVehicle() != cart) return;
            Vector v = cart.getVelocity();
            if (v.lengthSquared() > 0.001) cart.setVelocity(v.normalize().multiply(0.65));
            r.ready.put(p.getUniqueId(), r.tick + 100);
            p.setCooldown(Material.FEATHER, 100);
        }
    }
