...
final class MinecartRaceCommand {
    private final STEMCraftAPI api;
    private final MinecartRaceMiniGame owner;

    MinecartRaceCommand(STEMCraftAPI api, MinecartRaceMiniGame owner) {
        this.api = api;
        this.owner = owner;
    }

    void register() {
        String id = MinecartRaceMiniGame.NAMESPACE;
    ...
    private void execute(CommandContext ctx) {
        ...
        if (!publicAction && !ctx.hasPermission("stemcraft.command." + MinecartRaceMiniGame.NAMESPACE) && !ctx.getSender().isOp()) {
            ctx.returnError("Arena setup requires stemcraft.command." + MinecartRaceMiniGame.NAMESPACE);
            return;
        }
