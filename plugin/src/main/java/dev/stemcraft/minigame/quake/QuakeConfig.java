...
final class QuakeConfig {
    private final STEMCraftAPI api;
    private final QuakeMiniGame owner;
    private final ConfigFile config;

    QuakeConfig(STEMCraftAPI api, QuakeMiniGame owner, ConfigFile config) {
        this.api = api;
        this.owner = owner;
        this.config = config;
    }
    ...
    void save(MiniGameArena a) {
        ...
        s.set("kill-target", d.killTarget);
        s.set("reload-ticks", d.reloadTicks);
        config.save();
    }
    ...
    void loadArenas() {
        ...
        for (String id : arenas.getKeys(false)) {
            ...
            try {
                ...
                d.killTarget = s.getInt("kill-target", 20);
                d.reloadTicks = s.getInt("reload-ticks", 30);
                var validation = a.validate();
                if (validation.hasErrors())
                    api.messages().warn(QuakeMiniGame.NAMESPACE + ": " + id + ": " + String.join("; ", validation.getErrors()));
                ...
                api.messages().warn("Cannot load " + QuakeMiniGame.NAMESPACE + " arena " + id + ": " + ex.getMessage());
            }
        }
    }
