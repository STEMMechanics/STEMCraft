...
final class QuakeCommand {
    private final STEMCraftAPI api;
    private final QuakeMiniGame owner;

    QuakeCommand(STEMCraftAPI api, QuakeMiniGame owner) {
        this.api = api;
        this.owner = owner;
    }

    void register() {
        String id = QuakeMiniGame.NAMESPACE;
        ...
        for (String key : List.of("arena", "lobby", "spectator", "minplayers", "maxplayers", "roundseconds", "countdown", "killtarget", "reloadticks")) {
    ...
    private void execute(CommandContext ctx) {
        ...
        if (!publicAction && !ctx.hasPermission("stemcraft.command." + QuakeMiniGame.NAMESPACE) && !ctx.getSender().isOp()) {
            ctx.returnError("Arena setup requires stemcraft.command." + QuakeMiniGame.NAMESPACE);
            return;
        }
        ...
        QuakeArenaSettings d = owner.settings(a);
        switch (action) {
                ...
                switch (ctx.getArgLower(2)) {
                    ...
                    default -> {
                        ...
                        switch (ctx.getArgLower(2)) {
                            ...
                            case "killtarget" -> d.killTarget = n;
                            case "reloadticks" -> d.reloadTicks = n;
                            default -> {
