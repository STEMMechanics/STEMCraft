...
final class MobShooterCommand {
    private final STEMCraftAPI api;
    private final MobShooterMiniGame owner;

    MobShooterCommand(STEMCraftAPI api, MobShooterMiniGame owner) {
        this.api = api;
        this.owner = owner;
    }

    void register() {
        String id = MobShooterMiniGame.NAMESPACE;
    ...
    private void execute(CommandContext ctx) {
        ...
        if (!publicAction && !ctx.hasPermission("stemcraft.command." + MobShooterMiniGame.NAMESPACE) && !ctx.getSender().isOp()) {
            ctx.returnError("Arena setup requires stemcraft.command." + MobShooterMiniGame.NAMESPACE);
            return;
        }
