...
final class MobShooterArenaHandler implements MiniGameArenaHandler {
    ...
    MobShooterArenaHandler(STEMCraftAPI api, MobShooterMiniGame owner) {
        this.api = api;
        this.owner = owner;
    }

    private MobShooterArenaSettings settings(MiniGameArena a) {
        return owner.settings(a);
    }

    private String task(MiniGameArena a) {
        return "minigame:" + MobShooterMiniGame.NAMESPACE + ":" + a.id();
    }
    ...
    public void validate(@NotNull MiniGameArena a, ArenaValidationResult result) {
        MobShooterArenaSettings d = settings(a);
    ...
    public Location onPlayerJoinArena(MiniGameArena a, Player p) {
        ...
        api.messages().info(p, MobShooterMiniGame.INSTRUCTIONS);
    ...
    public void onArenaStatusChanged(MiniGameArena a, MiniGameArena.ArenaStatus oldStatus, MiniGameArena.ArenaStatus status) {
        if (status == STARTING) {
            ...
            rounds.put(a, r);
            a.set("winner", "-");
            r.objective = MobShooterMiniGame.INSTRUCTIONS;
            int i = 0;
    ...
    private void equip(Player p) {
        ...
        p.getInventory().setItem(0, targetBow());
        p.getInventory().setItem(9, new ItemStack(Material.ARROW, 64));
    }

    private ItemStack targetBow() {
        ItemStack item = new ItemStack(Material.BOW);
        var meta = item.getItemMeta();
        meta.displayName(Component.text("Target bow"));
    ...
    private void targetTick(MiniGameArena a, Round r) {
        for (Entity e : new ArrayList<>(r.entities)) {
            ...
            if (e instanceof AbstractArrow arrow && (arrow.isInBlock() || arrow.getTicksLived() > 100)) e.remove();
        }
        ...
        while (r.targets.size() < settings(a).targetCount) {
            ...
            LivingEntity target;
            Class<? extends LivingEntity> type = switch (ThreadLocalRandom.current().nextInt(3)) {
                case 0 -> Chicken.class;
                case 1 -> Rabbit.class;
                default -> Pig.class;
            };
            target = a.world().spawn(l, type);
        ...
        for (Player p : a.getPlayers()) p.getInventory().setItem(9, new ItemStack(Material.ARROW, 64));
    }
    ...
    void registerListeners() {
        ...
        api.events().register(EntityShootBowEvent.class, e -> {
            if (!(e.getEntity() instanceof Player p)) return;
            MiniGameArena a = owner.game.findPlayer(p);
            if (a == null) return;
            Round r = rounds.get(a);
            if (a.getStatus() != RUNNING || !a.hasPlayer(p) || r == null) {
                e.setCancelled(true);
                return;
            }
            r.entities.add(e.getProjectile());
    ...
    private void targetDamage(EntityDamageEvent event) {
        for (var entry : rounds.entrySet()) {
            ...
            if (hit.getDamager() instanceof AbstractArrow arrow && r.entities.contains(arrow)
                    && arrow.getShooter() instanceof Player p) {
                shooter = p;
                arrow.remove();
            }
            if (shooter == null || !a.hasPlayer(shooter)) return;
            int points = target instanceof Chicken ? 3 : target instanceof Rabbit ? 2 : 1;
    ...
    private void interact(PlayerInteractEvent event) {
        ...
        boolean bow = event.getItem() != null && event.getItem().getType() == Material.BOW;
        if (!bow || !a.hasPlayer(p) || a.getStatus() != RUNNING) event.setCancelled(true);
    }
