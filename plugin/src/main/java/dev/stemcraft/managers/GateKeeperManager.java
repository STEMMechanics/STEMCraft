...
import dev.stemcraft.api.services.GateKeeperService;
...
public final class GateKeeperManager implements GateKeeperService {
    ...
    public GateKeeperManager(STEMCraft plugin) {
        this.plugin = plugin;
        plugin.saveResource(GATEKEEPER_FILENAME, false);
    }

    public void onEnable() {
        config = plugin.getConfig(GATEKEEPER_FILENAME);

        enabled = config.getBoolean("enabled", true);

        String spawnLocationStr = config.getString("spawn");
        if(spawnLocationStr != null) {
            spawnLocation = SCLocation.deserialize(spawnLocationStr);
            if (spawnLocation == null) {
                plugin.error("Invalid gatekeeper location in config: " + spawnLocationStr);
            }
        }
        ...
        plugin.registerCommand("invite")
                .setDescription("Manage server invite codes")
                .setUsage("/invite add <code> <yyyy-MM-dd|never> | /invite remove <code> | /invite list")
                .addTabCompletion("add", "", "never")
                .addTabCompletion("remove", "{invite-codes}")
                ...
                .setExecutor((unused, cmd, ctx) -> {
                    ...
                    switch(sub) {
                        case "add" -> {
                            ...
                            if (expires != null && !SCTime.validDate(expires)) {
                        ...
                        case "list" -> {
                            ConfigurationSection section = config.getConfigurationSection("invites");
                            ...
                            SCChatMenu.render(ctx.getSenderAsPlayer(), "Invite Codes", "invite list", page, inviteCodes.size(), (start, count, isPlayer) -> {
                                        ...
                                        line = line.append(
                                                Component.text("[del]", NamedTextColor.RED)
                                                        ...
                                                        .hoverEvent(HoverEvent.showText(
                                                                Component.text(plugin.localeService().get("BOOK_LIST_GET_HOVER", "name", code))
                                                        ))
                                        );
                                    }
    ...
    private void gatePlayer(Player player) {
        SCPlayer.teleport(player, spawnLocation);
        SCPlayer.hide(player);
        ...
        plugin.messengerService().plain(player,
                "&e----------------------------------------------------\n" +
    ...
    private void releasePlayer(Player player) {
        ...
        if (releaseLocationStr != null) {
            Location releaseLocation = SCLocation.deserialize(releaseLocationStr);
            if (releaseLocation != null) {
                SCPlayer.teleport(player, releaseLocation);
                hasLocation = true;
            }
        }
    ...
    private boolean isCodeValid(String code) {
        ...
        ConfigurationSection invite = config.getConfigurationSection("invites." + code);
