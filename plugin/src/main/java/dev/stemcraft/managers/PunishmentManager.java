...
public class PunishmentManager implements PunishmentService {
    ...
    public PunishmentManager(STEMCraft plugin) {
        this.plugin = plugin;
        this.webService = plugin.webService();
    ...
    public void onEnable() {
        ...
        plugin.taskService().registerPersistentCallback("punishment", (type, id, data) -> {
            ...
            if(record.type().equalsIgnoreCase("ban")) {
                plugin.messengerService().broadcast("BAN_EXPIRED_BROADCAST", player, "player", record.targetName());
            } else if(record.type().equalsIgnoreCase("mute")) {
                plugin.messengerService().broadcast("MUTE_EXPIRED_BROADCAST", "player", record.targetName());
            }
        });
        ...
        plugin.registerCommand("kick")
                ...
                .setExecutor((ignored, cmd, ctx) -> {
                    ...
                    punish.record(target.getUniqueId(), actor, null, "kick", true, reason);
        ...
        plugin.registerCommand("warn")
                .setPermission("stemcraft.command.warn")
                .addTabCompletion("{player}", "{reason}")
                .setExecutor((ignored, cmd, ctx) -> {
                    if (ctx.args().isEmpty()) {
                        cmd.error(ctx.getSender(), cmd.getUsage());
                        return;
                    ...
                    Player actor = ctx.getSenderAsPlayer();
                    punish.record(target.getUniqueId(), actor, null, "warn", false, reason);
        ...
        plugin.registerCommand("ban")
                .setPermission("stemcraft.command.ban")
                .addTabCompletion("{player}", "unban", "{reason}")
                .addTabCompletion("{player}", "{duration}", "{reason}")
                .addTabCompletion("{player}", "perm", "{reason}")
                .addTabCompletion("{player}", "{reason}")
                .setExecutor((ignored, cmd, ctx) -> {
                    ...
                    punish.record(targetUuid, actor, duration, "ban", true, reason);
                    ...
                    if(pardon) {
                        ...
                        plugin.messengerService().broadcast("BAN_EXPIRED_BROADCAST", target.getPlayer(), "reason", reason, "actor", ctx.getSenderName());
                    } else {
                        ...
                        String durationString = duration == null ? "permanently" : "for " + SCTime.formatDuration(duration.toSeconds());
                        plugin.messengerService().broadcast("BAN_PLAYER_BROADCAST", target.getPlayer(), "reason", reason, "actor", ctx.getSenderName(), "duration", durationString);
                    }
                })
                .register(STEMCraft.getInstance());
    }
    ...
    public synchronized void record(UUID playerUuid, Player actor, Duration duration, String type, boolean alerted, String reason) {
        String playerName = SCPlayer.name(playerUuid);
        if(playerName == null) {
    ...
    private synchronized void save() {
        try {
            ...
            plugin.getLogger().warning("Failed to save punishments.yml: " + e.getMessage());
        }
    }
    ...
    private void loadFromConfig() {
        ...
        ConfigurationSection section = data.getConfigurationSection("punishments");
        if (section == null) {
            return;
        }
    ...
    private void writeRecordToConfig(PunishmentRecord record) {
        ...
        data.set(path + ".target.uuid",
                record.targetUuid() != null ? record.targetUuid().toString() : null);
        data.set(path + ".target.name", record.targetName());
        data.set(path + ".actor.uuid",
                record.actorUuid() != null ? record.actorUuid().toString() : null);
        data.set(path + ".actor.name", record.actorName());
        data.set(path + ".type", record.type());
        data.set(path + ".alerted", record.alerted());
        data.set(path + ".reason", record.reason());
        data.set(path + ".createdAt", record.createdAt().toEpochMilli());
        if (!record.permanent()) {
            data.set(path + ".durationSeconds", record.durationSeconds());
        } else {
            data.set(path + ".durationSeconds", null);
        }
    }
    ...
    private void registerWebEndpoint() {
        ...
        webService.registerEndpointHandler("/punish", (method, uri, queryParams) -> {
            ...
            String title = "Punishments";
            String basePath = "/punish";
            ...
            for (PunishmentRecord p : pageData) {
                ...
                sb.append("<td>").append(SCTime.toFriendlyTime(p.createdAt())).append("</td>");
                ...
                if (p.permanent()) {
                    ...
                    sb.append("<td>").append(SCTime.formatDuration(p.durationSeconds())).append("</td>");
                }
