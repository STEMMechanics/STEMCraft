...
public class Parkour {
    ...
    public void onEnable(STEMCraftAPI api) {
        this.api = api;
        config = api.config().get(CONFIG_FILENAME);
    ...
    private void loadConfig() {
        ...
        if (arenas != null) {
            for (String arenaId : arenas.getKeys(false)) {
                loadArena(arenaId, arenas.getSection(arenaId));
            }
        }
    }
     ...
     */
    private void loadArena(String arenaId, ConfigSection section) {
        if(section == null) {
        ...
        String lobbyRegionString = section.getString("lobby");
        if(lobbyRegionString == null) {
            api.warn("Arena '" + arenaId + "' does not have a lobby region defined.");
            return;
        }

        SCRegion lobbyRegion = SCRegion.fromString(lobbyRegionString, world);
        if(lobbyRegion == null) {
            api.warn("Lobby region for arena '" + arenaId + "' is invalid.");
            return;
        }

        String finishRegionString = section.getString("finish");
        if(finishRegionString == null) {
            api.warn("Arena '" + arenaId + "' does not have an finish region defined.");
            return;
        }

        SCRegion finishRegion = SCRegion.fromString(finishRegionString, world);
        if(finishRegion == null) {
            api.warn("Finish region for arena '" + arenaId + "' is invalid.");
            return;
        }
        ...
        arena.set("lobbyRegion", lobbyRegion);
        arena.set("finishRegion", finishRegion);

        api.regions().addListener(regionListenerPrefix + "lobby", lobbyRegion, new RegionListener() {
            @Override
            public void onExit(Player player, SCRegion region) {
            if(arena.hasPlayer(player)) {
                startParkourRun(arena, player);
            }
            }
        });

        api.regions().addListener(regionListenerPrefix + "finish", finishRegion, new RegionListener() {
            @Override
            public void onEnter(Player player, SCRegion region) {
                if(arena.hasPlayer(player)) {
                    endParkourRun(arena, player, true);
                }
            }
        });

        arena.setStatus(MiniGameArena.STATUS_RUNNING);
    ...
    private void unloadArena(MiniGameArena arena) {
        ...
        api.regions().Listener(regionListenerPrefix + "lobby");
        api.regions().Listener(regionListenerPrefix + "finish");
    ...
    private void loadHUDs() {
        api.minigames().registerHud(NAMESPACE, MiniGameArena.STATUS_RUNNING,
                List.of(
                        "Parkour: {arena-name}",
                        "Best time: {arena-best-time} by {arena-best-player}"
                ),
                List.of(
                        "Parkour: {arena-name}",
                        "",
                ...
                (arena, player) -> {
                    ...
                    map.put("arena-best-time", config.getInt("arenas." + arena.getId() + ".1.time", 0));
    ...
    private void loadCommand() {
        ...
        api.commands().create("parkour")
                ...
                .usage("/parkour <join|leave|stop|start|restart> [arena]")
                .tabCompletion("join", "{parkour-arenas}", "{player}")
                .tabCompletion("leave", "{player}")
                .tabCompletion("stop", "{parkour-arenas}")
                .tabCompletion("start", "{parkour-arenas}")
                .tabCompletion("restart", "{parkour-arenas}")
                .tabCompletion("enable", "{parkour-arenas}")
                .tabCompletion("disable", "{parkour-arenas}")
                .tabCompletion("create")
                .tabCompletion("set", "lobbyspawn")
                .tabCompletion("set", "lobbyregion")
                .tabCompletion("set", "parkourregion")
                .tabCompletion("set", "finishregion")
    ...
    private void startParkourRun(MiniGameArena arena, Player player) {
        arena.info(player, "You have started a parkour run! Reach the finish area as fast as you can!");
        long startTime = System.currentTimeMillis();
        arena.getPlayer(player).set("startTime", startTime);
    }

    private void endParkourRun(MiniGameArena arena, Player player, boolean reachedFinish) {
        MiniGamePlayer mgPlayer = arena.getPlayer(player);
        Long startTime = mgPlayer.get("startTime", Long.class);
        if (startTime == null) {
            return;
        }
        ...
        if (reachedFinish) {
            arena.info(player, "Congratulations! You have completed the parkour run in " + formatDuration(duration) + "!");
        } else {
            arena.info(player, "You have ended your parkour run.");
        }

        mgPlayer.remove("startTime");
    }
}
