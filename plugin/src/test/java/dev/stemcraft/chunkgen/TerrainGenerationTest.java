...
class TerrainGenerationTest {
    ...
    @AfterAll static void cleanup() { blockData.close(); MockBukkit.unmock(); }
    ...
    private World world(long seed, int min, int max) {
        ...
        when(world.getSeed()).thenReturn(seed); when(world.getMinHeight()).thenReturn(min); when(world.getMaxHeight()).thenReturn(max);
        return world;
    }
    @Test void cavernLightsAreDeterministicAtNegativeCoordinatesAndUseSupportedSurfaces() {
        ...
        for (int cx = -3; cx <= 0; cx++) {
            var first = litCavern(); var second = litCavern();
    ...
    @Test void chunksAreOrderIndependentConcurrentAndMatchHeightQueries() throws Exception {
        ...
        for (String id : MODELS.keySet()) {
            ...
            for (int x = 0; x < 16; x += 3) for (int z = 0; z < 16; z += 3) {
                int expected = -64;
                for (int y = 319; y >= -64; y--) if (first.getType(x, y, z) != Material.AIR) { expected = y + 1; break; }
            ...
            for (int x = -5000; x < 5000; x += 250) assertTrue(provider.getBiomes(world).contains(provider.getBiome(world, x, 20, -x)));
        }
    }
    @Test void everyGeneratorHasSolidSpawnFloorAndHeadroomAcrossSeedsAndHeightRanges() {
        for (String id : MODELS.keySet()) for (long seed : new long[]{0, -1, 29381}) for (int[] range : new int[][]{{-64,320},{0,256}}) {
            var generator = generator(id);
            ...
            if (id.equals("skylands")) for (int x = 0; x < 16; x++) for (int z = 0; z < 16; z++)
                assertEquals(Material.AIR, data.getType(x, range[0], z), "Skylands must have no ground plane");
        }
    }
    @Test void skylandsOreFeatureIncludesAllRequiredResourceTypes() {
    ...
    @Test void surfaceTreesAreDeterministicAndStayInsideTheirPlacementCell() {
        ...
        for (int chunk = -2; chunk <= 2; chunk++) {
            ...
            first.biome = Biome.FOREST; second.biome = Biome.FOREST;
            for (int x = 0; x < 16; x++) for (int z = 0; z < 16; z++) {
                first.setBlock(x, 90, z, Material.GRASS_BLOCK); second.setBlock(x, 90, z, Material.GRASS_BLOCK);
            }
            feature.generate(context, chunk, -1, first); feature.generate(context, chunk, -1, second);
            assertArrayEquals(first.blocks, second.blocks);
            if (Arrays.asList(first.blocks).contains(Material.OAK_LOG)) woodedChunks++;
            for (int x = 0; x < 16; x++) for (int y = 91; y < 100; y++) {
                assertEquals(Material.AIR, first.getType(x, y, 0)); assertEquals(Material.AIR, first.getType(x, y, 15));
                assertEquals(Material.AIR, first.getType(0, y, x)); assertEquals(Material.AIR, first.getType(15, y, x));
            }
        }
        assertTrue(woodedChunks > 0);
    }
    @Test void terrainFieldsHaveStableCurrentFingerprints() {
        // Fixed snapshots guard accidental algorithm changes; populate constants only after reviewing a new version.
        Map<String, Long> hashes = new TreeMap<>();
        for (String id : MODELS.keySet()) {
            TerrainModel model = MODELS.get(id).apply(new GenerationContext(83423, -64, 320, new NamespacedKey("stemcraft", id), 1));
            long hash = 1;
            for (int x = -3072; x <= 3072; x += 384) for (int z = -2048; z <= 2048; z += 512) {
                var column = model.column(x, z);
                for (int y = -64; y < 320; y += 24) hash = hash * 31 + Double.doubleToLongBits(column.density(y));
            }
            hashes.put(id, hash);
        }
        assertEquals(Map.of("deep", -8760065451613218688L, "faraway", -1888401393809985420L,
                "skylands", 386427282392049432L, "wasteland", -3392173478962655906L), hashes);
    }
    @Test void deepHasAnUnbrokenLowerRoofAndSafeUndergroundSpawn() {
        for (long seed : new long[]{0, -1, 83423}) {
            ...
            for (int x = -2048; x <= 2048; x += 128) for (int z = -1024; z <= 1024; z += 128) {
                var column = model.column(x, z);
    ...
    @Test void skylandsHasWideVoidGapsAndExplorableIslandGroups() {
        ...
        for (long seed : new long[]{0, 83423}) {
            ...
            for (int x = -4096; x <= 4096; x += 128) for (int z = -4096; z <= 4096; z += 128) {
                boolean solid = false;
                var column = model.column(x, z);
    ...
    @Test void detailsPopulateDryGroundAndCaveFloorsDeterministically() {
        var feature = new dev.stemcraft.chunkgen.feature.SurfaceDetailFeature();
        for (Biome biome : List.of(Biome.DESERT, Biome.LUSH_CAVES)) {
            var a = new MemoryChunk(-64, 320); var b = new MemoryChunk(-64, 320);
            a.biome = biome; b.biome = biome;
            Material floor = biome == Biome.DESERT ? Material.SAND : Material.STONE;
            a.setRegion(0, 50, 0, 16, 51, 16, floor); b.setRegion(0, 50, 0, 16, 51, 16, floor);
            var context = new GenerationContext(83423, -64, 320, new NamespacedKey("stemcraft", "deep"), 1);
            feature.generate(context, -20, 4, a); feature.generate(context, -20, 4, b);
    ...
    @Test void oasesProvideWaterAndFertileLand() {
        ...
        assertTrue(fertile > 0); assertTrue(water > 0);
    }
    @Test void chunksAndSurfaceDetailsAreOrderIndependent() throws Exception {
    ...
    @Test void vegetationReachesLowerIslandBandsAndKeepsHeadroom() {
        var feature = new dev.stemcraft.chunkgen.feature.SurfaceVegetationFeature();
        var context = new GenerationContext(83423, -64, 320, new NamespacedKey("stemcraft", "skylands"), 1);
        int wooded = 0;
        for (int chunk = -5; chunk < 5; chunk++) {
            var data = new MemoryChunk(-64, 320); data.biome = Biome.FOREST;
            ...
            assertEquals(logs[2], logs[0]); assertEquals(logs[2], logs[1]);
            wooded += logs[0];
        }
        assertTrue(wooded > 0);
    }
    @Test void builtInsRegisterOneCurrentImplementationPerType() {
    ...
    static final class MemoryChunk implements ChunkGenerator.ChunkData {
        ...
        MemoryChunk(int min, int max) { this.min = min; this.max = max; blocks = new Material[256 * (max - min)]; Arrays.fill(blocks, Material.AIR); }
        private int index(int x, int y, int z) { return ((x * 16 + z) * (max - min)) + y - min; }
        @Override public int getMinHeight() { return min; }
        @Override public int getMaxHeight() { return max; }
        @Override public Material getType(int x, int y, int z) { return blocks[index(x,y,z)]; }
        @Override public void setBlock(int x, int y, int z, Material material) {
            assertTrue(x >= 0 && x < 16 && z >= 0 && z < 16 && y >= min && y < max);
            blocks[index(x,y,z)] = material;
        }
        @Override public void setBlock(int x, int y, int z, BlockData data) { setBlock(x,y,z,data.getMaterial()); }
        @Override public void setBlock(int x, int y, int z, MaterialData data) { setBlock(x,y,z,data.getItemType()); }
        @Override public void setRegion(int x0,int y0,int z0,int x1,int y1,int z1,Material m) {
            for(int x=x0;x<x1;x++) for(int y=y0;y<y1;y++) for(int z=z0;z<z1;z++) setBlock(x,y,z,m);
        }
        @Override public void setRegion(int x0,int y0,int z0,int x1,int y1,int z1,BlockData d) { setRegion(x0,y0,z0,x1,y1,z1,d.getMaterial()); }
        @Override public void setRegion(int x0,int y0,int z0,int x1,int y1,int z1,MaterialData d) { setRegion(x0,y0,z0,x1,y1,z1,d.getItemType()); }
        @Override public Biome getBiome(int x, int y, int z) { return biome; }
        @Override public BlockData getBlockData(int x, int y, int z) { return getType(x,y,z).createBlockData(); }
        @Override public MaterialData getTypeAndData(int x, int y, int z) { return new MaterialData(getType(x,y,z)); }
        @Override public byte getData(int x, int y, int z) { return 0; }
        @Override public int getHeight(HeightMap map, int x, int z) { throw new UnsupportedOperationException(); }
    }
}
