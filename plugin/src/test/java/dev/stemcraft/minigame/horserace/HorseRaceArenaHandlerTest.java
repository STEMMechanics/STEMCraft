...
class HorseRaceArenaHandlerTest {
    private ServerMock server;
    private WorldMock world;
    private STEMCraftAPI api;
    private MiniGameArena arena;
    private HorseRaceArenaSettings settings;
    private List<Player> players;
    private HorseRaceMiniGame game;
    private HorseRaceArenaHandler handler;
    private MiniGameArena.ArenaStatus status;

    @BeforeEach
    void setup() {
        server = MockBukkit.mock();
        world = server.addSimpleWorld("horserace");
        api = mock(STEMCraftAPI.class, RETURNS_DEEP_STUBS);
        arena = mock(MiniGameArena.class);
        settings = new HorseRaceArenaSettings();
        players = new ArrayList<>();
        when(arena.getOrCreate(eq("horseraceSettings"), eq(HorseRaceArenaSettings.class), any())).thenReturn(settings);
    ...
    private void game() {
        game = new HorseRaceMiniGame(api);
        game.game = mock(MiniGame.class);
        handler = new HorseRaceArenaHandler(api, game);
    }
    ...
    @Test
    void raceRequiresOrderedNonOverlappingGates() {
        game();
        settings.checkpoints.add(region(1, 63, 1, 2, 66, 3));
        settings.checkpoints.add(region(2, 63, 1, 3, 66, 3));
        var result = ArenaValidationResult.success();
        handler.validate(arena, result);
        assertTrue(result.getErrors().stream().anyMatch(s -> s.contains("overlap")));
    }

    @Test
    void horseRaceRequiresEveryGateBeforeFinishing() {
        raceGates();
    }
    ...
    private void raceGates() {
        ...
        players.add(rider);
        org.bukkit.entity.Entity mount = mock(org.bukkit.entity.Horse.class, RETURNS_DEEP_STUBS);
    ...
    void usesFrameworkDirectlyAndPersistsOnlyItsOwnSettings() {
        assertSame(dev.stemcraft.minigame.BaseMiniGame.class, HorseRaceMiniGame.class.getSuperclass());
        game();
        ...
        new HorseRaceConfig(api, game, file).save(arena);
