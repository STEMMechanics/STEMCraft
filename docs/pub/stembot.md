...
You can interrupt a tour with a recognised topic. Say `bye`, `stop`, or `go away`, or run `/stembot close` to dismiss the guide. Walking far away returns you to public chat; coming back close can resume the conversation while the session remains open. Abandoned sessions eventually close.

STEMBot recognises configured topics rather than answering every possible sentence. If a phrase does not work, try a short topic or one of the example replies it shows.
